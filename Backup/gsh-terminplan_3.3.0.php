 ...
 *
 * Alle Plugin-Optionen werden beim ersten Aufruf einmalig aus der Datenbank
 * geladen und für den Rest des Requests in einer statischen Variable gehalten.
 * Das reduziert wiederholte get_option()-Aufrufe im Rendering-Pfad
 * (Shortcode, Quartalsgrenzen, Kategorie-Mapping).
 *
 * @since 3.3.0
 ...
 */
function gsh_tp_opt( $key, $default = '' ) {
    static $c = null;
    if ( null === $c ) {
        $c = array(
...
function gsh_tp_settings_page() {
    ...
    // Cache als veraltet markieren → nächster Seitenaufruf triggert Hintergrund-Refresh
    ...
    if ( isset( $_POST['gsh_tp_sync'] ) &&
        ...
        if ( $ok ) {
            ...
            echo '<div class="notice notice-error"><p>'
               ...
               . 'bitte die iCal-URL pr&uuml;fen oder warten, bis der IServ-Server erreichbar ist.'
               . '</p></div>';
        }
    }
    ?>
    <div class="wrap">
        ...
        <table class="form-table" style="max-width:700px">
            ...
            <tr>
                ...
                <td>
                    <?php
                    $has_data  = ! empty( get_option( GSH_TP_CACHE_KEY, '' ) );
                    $is_fresh  = false !== get_transient( GSH_TP_FRESH_KEY );
        ...
        <form method="post">
            ...
            <p>
                ...
                <span class="description" style="margin-left:10px">
                    Markiert den Cache als veraltet. Beim n&auml;chsten Seitenaufruf startet ein Hintergrund-Refresh &ndash; kein Besucher wartet.
                </span>
            </p>
        </form>
 ...
 *
 * Die eigentlichen Daten liegen in der permanenten Option GSH_TP_CACHE_KEY
 * und laufen nie ab. Ein separater Freshness-Transient (GSH_TP_FRESH_KEY)
 * steuert wann ein Refresh nötig ist. Ist er abgelaufen, wird ein WP-Cron-Job
 * eingeplant – der Besucher wartet jedoch nicht auf den Download von IServ.
 *
 * Nur beim allerersten Aufruf (keine Daten gespeichert) wird synchron geladen.
 *
 * @since 1.2.0 (Stale-While-Revalidate seit 3.3.0)
...
function gsh_tp_fetch_ical() {
    ...
    if ( false === get_transient( GSH_TP_FRESH_KEY ) ) {
        gsh_tp_schedule_refresh();
    }

    // 3. Erstinstallation: noch keine Daten → einmalig synchron laden
    if ( empty( $data ) ) {
        $data = gsh_tp_fetch_sync();
    }
...
function gsh_tp_fetch_sync() {
    ...
    update_option( GSH_TP_CACHE_KEY, $body, false );
 ...
 *
 * Setzt den Freshness-Transient sofort auf 120 Sekunden (Schutzpuffer), damit
 * mehrere gleichzeitige Besucher bei abgelaufenem Cache nicht alle einen
 * eigenen Cron einplanen. spawn_cron() startet den Cron non-blocking.
 *
 * @since 3.3.0
 * @return void
 */
function gsh_tp_schedule_refresh() {
    // Schutz-Transient: verhindert Doppel-Scheduling für 120 Sekunden
    set_transient( GSH_TP_FRESH_KEY, time(), 120 );
    if ( ! wp_next_scheduled( 'gsh_tp_cron_refresh' ) ) {
        wp_schedule_single_event( time(), 'gsh_tp_cron_refresh' );
    }
...
function gsh_tp_build_map() {
    $raw = gsh_tp_opt( 'kat_mapping' ) ?: gsh_tp_default_mapping();
    $r   = array();
...
function gsh_tp_shortcode( $atts ) {
    ...
    $sjs     = gsh_tp_opt( 'schuljahr_start', '2025-08-25' );
    ...
    $last_sync = gsh_tp_opt( 'last_sync' );
    if ( $last_sync ) {
        $dt = new DateTime( $last_sync, new DateTimeZone( 'UTC' ) );
        $dt->setTimezone( wp_timezone() );
